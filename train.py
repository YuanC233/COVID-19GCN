...
if __name__ == '__main__':
    ...
    for fold in range(len(folds)):
        train_data = get_data(f'train_cv/fold_{fold}/train.csv', device='cuda:0', no_h=NO_H)
        dev_data = get_data(f'train_cv/fold_{fold}/dev.csv', device='cuda:0', no_h=NO_H)
        test_data = get_data(f'train_cv/fold_{fold}/dev.csv', device='cuda:0', no_h=NO_H)
        ...
        for idx, params in enumerate(HYPER_PARA):
            ...
            data_loader = DataLoader(train_data, batch_size=BATCH_SIZE, shuffle=True, collate_fn=collate)
        ...
        _test_data_loader = DataLoader(dev_data, batch_size=32, shuffle=False, collate_fn=collate)
